...
int userInputZA = 0;
int sensorTofA = 0;
int wifiHeartbeatA = 0;
int countA = 0;

// Variables for Program B
...
void outputToSoftwareA() {
  Serial.print("stopValA: ");
  Serial.print(stopValA);
  Serial.print(", startValA: ");
  Serial.print(startValA);
  Serial.print(", sensorGyroXA: ");
  Serial.print(sensorGyroXA);
  Serial.print(", sensorGyroYA: ");
  Serial.print(sensorGyroYA);
  Serial.print(", sensorGyroZA: ");
  Serial.print(sensorGyroZA);
  Serial.print(", userInputXA: ");
  Serial.print(userInputXA);
  Serial.print(", userInputYA: ");
  Serial.print(userInputYA);
  Serial.print(", userInputZA: ");
  Serial.print(userInputZA);
  Serial.print(", sensorTofA: ");
  Serial.print(sensorTofA);
  Serial.print(", wifiHeartbeatA: ");
  Serial.println(wifiHeartbeatA);

  countA++;
  if (countA > 1) countA = 0;
  stopValA = countA;
  startValA = countA;
  sensorGyroXA = countA*255;
  sensorGyroYA = countA*255;
  sensorGyroZA = countA*255;
  userInputXA = countA*255;
  userInputYA = countA*255;
  userInputZA = countA*255;
  sensorTofA = countA * (2970) + 30;
  wifiHeartbeatA = countA;
}
